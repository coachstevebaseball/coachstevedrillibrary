...
## Access Model Restructure — Public Drill Library + Admin-Only Backend
- [x] Remove AthletePortal route and import from App.tsx
- [x] Remove MyProfile, AcceptInvite, VerifyEmail routes from App.tsx
- [x] Change all coach-protected routes to admin-only (requiredRole="admin")
- [x] Remove "My Drills" button and athlete-portal link from Home.tsx nav
- [x] Remove prominent Login/Logout buttons for non-admin users from Home.tsx
- [x] Add subtle admin login link to Home.tsx nav
- [x] Remove auth-gate from DrillsDirectory.tsx (no login required)
- [x] Remove "Account Inactive" check from DrillsDirectory.tsx
- [x] Remove athlete-portal link from DrillsDirectory.tsx nav
- [x] Remove Login button from DrillsDirectory.tsx nav
- [x] Remove hasAccess check from DrillDetail.tsx — all content public
- [x] Remove "Client Access Required" lock screen from DrillDetail.tsx
- [x] Remove PREVIEW_MODE logic from DrillDetail.tsx
- [x] Remove athlete-only Q&A section from DrillDetail.tsx
- [x] Keep admin edit controls on DrillDetail.tsx (only visible to admin)
- [x] Verify admin routes still require authentication
- [x] Test public access to drill library without login
- [x] Test admin access to dashboard with login
