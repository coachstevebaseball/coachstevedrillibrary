...
// Bulk Email Types and Functions
...
export async function sendBulkEmail(data: BulkEmailData): Promise<{ success: boolean; error?: string }> {
  if (!ENV.resendApiKey) {
    console.warn("[Email] Resend API key not configured, skipping email");
    return { success: false, error: "Email service not configured" };
  }
  ...
  try {
    const emailHtml = generateBulkEmailHtml(data);
    ...
    if (result.error) {
      console.error("[Email] Failed to send bulk email:", result.error);
      return { success: false, error: result.error.message };
    }

    console.log("[Email] Bulk email sent successfully to", data.recipientEmail);
    return { success: true };
  } catch (error) {
...
function generateBulkEmailHtml(data: BulkEmailData): string {
    ...
    <html>
      <head>
        ...
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, #1e3a5f 0%, #0f172a 100%); color: white; padding: 30px; border-radius: 8px 8px 0 0; text-align: center; }
          .header h1 { margin: 0; font-size: 24px; font-weight: bold; }
          .content { background: #f9fafb; padding: 30px; border-radius: 0 0 8px 8px; border: 1px solid #e5e7eb; border-top: none; }
          .message-box { background: white; padding: 25px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #c41e3a; }
          .footer { text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; font-size: 12px; color: #666; }
          .cta-button { display: inline-block; background: #c41e3a; color: white; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: bold; margin: 20px 0; }
          .button-container { text-align: center; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            <h1>Coach Steve's Baseball Drills</h1>
          </div>
          ...
          <div class="content">
            ...
            <div class="button-container">
              <a href="https://coachstevemobilecoach.com" class="cta-button">Visit Your Portal</a>
            </div>
          ...
          <div class="footer">
            <p>© 2026 Coach Steve Baseball — Drills Directory. All rights reserved.</p>
            <p>You're receiving this email because you're a member of Coach Steve's Baseball Drills platform.</p>
          </div>
        </div>
      </body>
    </html>
